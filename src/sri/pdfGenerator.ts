...
export function generateInvoicePdfBuffer(invoice: Invoice, config: EmitterConfig): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    try {
      ...
      const emitterName = config.razonSocial || 'JOLUS SERVICES';
      const tradeName = config.nombreComercial || 'JOLUS SERVICES';
      ...
      // Footer message
      doc.fontSize(8).font('Helvetica-Oblique').fillColor('#64748b').text('Comprobante Electrónico emitido en conformidad con la normativa del SRI Ecuador. JOLUS SERVICES', 30, 780, { align: 'center', width: 535 });
