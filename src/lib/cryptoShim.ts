...
function sha256Bytes(str: string): Uint8Array {
  ...
  rv.setInt32(0, h0); rv.setInt32(4, h1); rv.setInt32(8, h2); rv.setInt32(12, h3);
  rv.setInt32(16, h4); rv.setInt32(20, h5); rv.setInt32(24, h6); rv.setInt32(28, h7);
  return result;
}
...
export const createHash = (algorithm: string) => {
  let buffer = '';
  const hashObj = {
    update: (data: any, _encoding?: string) => {
    ...
    digest: (encoding?: string) => {
      const bytes = sha256Bytes(buffer);
      if (encoding === 'base64') {
