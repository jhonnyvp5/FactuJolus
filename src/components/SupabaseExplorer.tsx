...
export const SupabaseExplorer: React.FC = () => {
  ...
  const getPrimaryKeyField = (tableName: string, row: any): { key: string; value: any } => {
    if (!row) return { key: 'id', value: undefined };
    if (tableName === 'clientes' && row.identificacion) return { key: 'identificacion', value: row.identificacion };
    if (tableName === 'productos' && row.codigo) return { key: 'codigo', value: row.codigo };
    if (tableName === 'emisor_config' && row.ruc) return { key: 'ruc', value: row.ruc };
    if (tableName === 'usuarios_portal' && row.correo) return { key: 'correo', value: row.correo };
    if (tableName === 'invitaciones' && row.id) return { key: 'id', value: row.id };
    if (tableName === 'invitaciones' && row.correo) return { key: 'correo', value: row.correo };
    if (row.id !== undefined) return { key: 'id', value: row.id };
    if (row.secuencial !== undefined) return { key: 'secuencial', value: row.secuencial };
    
    // Fallback first key
    const firstKey = Object.keys(row)[0] || 'id';
  ...
  const handleOpenEdit = (row: any) => {
    setEditingRow(row);
    setEditJsonText(JSON.stringify(row, null, 2));
    setModalError(null);
  ...
  const handleOpenInsert = () => {
    ...
    if (rows.length > 0) {
      ...
      Object.keys(sample).forEach(k => {
        ...
        if (typeof sample[k] === 'number') template[k] = 0;
        else if (typeof sample[k] === 'boolean') template[k] = false;
        else if (typeof sample[k] === 'object' && sample[k] !== null) template[k] = sample[k];
        else template[k] = '';
      });
      if ('id' in template) template.id = `id-${Date.now()}`;
    } else {
      template = { id: `id-${Date.now()}`, nombre: 'Ejemplo' };
    }
    setEditJsonText(JSON.stringify(template, null, 2));
    setModalError(null);
  ...
  const handleSaveModal = async () => {
    ...
    let parsedData: any;
    try {
      parsedData = JSON.parse(editJsonText);
    } catch (err: any) {
      setModalError(`JSON inválido: ${err.message}`);
      setSavingRow(false);
      return;
    }

    const { key } = getPrimaryKeyField(activeTab, parsedData);
    const conflictField = key !== 'id' ? key : undefined;

    const res = await saveRowToSupabaseTable(activeTab, parsedData, conflictField);
    setSavingRow(false);

    if (res.success) {
      setIsEditModalOpen(false);
      setIsInsertModalOpen(false);
      showNotification(`¡Registro ${isInsertModalOpen ? 'insertado' : 'actualizado'} con éxito en "${activeTab}"!`);
      await loadData();
    } else {
      setModalError(`Error de Supabase: ${res.errorDetails || 'Error al guardar.'}`);
    }
  };
  ...
  return (
    <div className="bg-white dark:bg-zinc-900 rounded-2xl shadow-sm border border-gray-100 dark:border-zinc-800 p-5 md:p-6 space-y-5">
        ...
        <div>
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-xl font-bold text-gray-900 dark:text-zinc-50">
              Explorador & Gestor CRUD de Supabase (12 Tablas)
            </h3>
        ...
        <div className="flex items-center gap-2">
          {activeTab !== 'storage_buckets' && (
            <button
              ...
              className="py-2 px-3.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl font-bold text-xs flex items-center gap-1.5 transition cursor-pointer shadow-xs"
            >
              <Plus className="w-4 h-4" />
              Insertar Datos en "{activeTab}"
            </button>
          )}
          ...
          <button
            onClick={loadData}
            disabled={loading}
            className="py-2 px-4 bg-indigo-50 dark:bg-indigo-950/40 hover:bg-indigo-100 dark:hover:bg-indigo-900/60 text-indigo-700 dark:text-indigo-300 rounded-xl font-medium text-xs flex items-center gap-2 transition cursor-pointer border border-indigo-100 dark:border-indigo-800/40"
          >
      ...
      {/* NOTIFICATIONS */}
      {actionSuccess && (
        <div className="bg-emerald-50 dark:bg-emerald-950/40 text-emerald-800 dark:text-emerald-300 p-3 rounded-xl border border-emerald-200 dark:border-emerald-800 text-xs flex items-center gap-2 font-bold animate-fade-in">
      ...
      {loading ? (
        <div className="py-12 text-center text-xs text-gray-400 font-mono animate-pulse">
          Sincronizando con Supabase en tiempo real...
        </div>
      ) : activeTab === 'storage_buckets' ? (
        /* BUCKET FILES LIST */
        ...
        <div className="space-y-4">
          {filteredRows.length === 0 ? (
            <div className="py-10 text-center text-xs text-gray-400 font-mono bg-gray-50 dark:bg-zinc-950 rounded-xl border border-dashed border-gray-200 dark:border-zinc-800 space-y-2">
              <p>No se encontraron datos registrados en la tabla "{activeTab}".</p>
              <button
                onClick={handleOpenInsert}
                className="px-3 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-lg text-xs inline-flex items-center gap-1 cursor-pointer"
              >
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse text-xs">
                <thead>
                  <tr className="border-b border-gray-200 dark:border-zinc-800 bg-gray-50 dark:bg-zinc-950 text-gray-500 dark:text-zinc-400 font-semibold">
                    {Object.keys(filteredRows[0] || {}).slice(0, 5).map((key) => (
                      <th key={key} className="py-2.5 px-3 capitalize">{key.replace('_', ' ')}</th>
                    ))}
                    <th className="py-2.5 px-3 text-right">Acciones CRUD</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-100 dark:divide-zinc-800 font-mono">
                      ...
                      <td className="py-2.5 px-3 text-right font-sans">
                        <div className="flex items-center justify-end gap-1.5">
                          <button
                            ...
                            className="px-2 py-1 bg-indigo-50 dark:bg-indigo-950/40 text-indigo-700 dark:text-indigo-300 rounded-lg text-[11px] font-semibold hover:bg-indigo-100 transition cursor-pointer flex items-center gap-1"
                          ...
                          <button
                            ...
                            className="px-2 py-1 bg-amber-50 dark:bg-amber-950/40 text-amber-700 dark:text-amber-300 rounded-lg text-[11px] font-semibold hover:bg-amber-100 transition cursor-pointer flex items-center gap-1"
                            title="Editar fila"
                          >
                          ...
                          <button
                            ...
                            className="px-2 py-1 bg-rose-50 dark:bg-rose-950/40 text-rose-700 dark:text-rose-300 rounded-lg text-[11px] font-semibold hover:bg-rose-100 transition cursor-pointer flex items-center gap-1"
                            title="Borrar de la tabla"
                          >
      ...
      {(isEditModalOpen || isInsertModalOpen) && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4 z-50">
          <div className="bg-white dark:bg-zinc-900 rounded-2xl max-w-3xl w-full p-6 space-y-4 shadow-2xl border border-gray-100 dark:border-zinc-800 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between border-b border-gray-100 dark:border-zinc-800 pb-3">
              <h4 className="font-bold text-sm text-gray-900 dark:text-zinc-100 flex items-center gap-2">
                {isInsertModalOpen ? <Plus className="w-4 h-4 text-emerald-500" /> : <Edit3 className="w-4 h-4 text-amber-500" />}
                {isInsertModalOpen ? `Insertar Nuevo Registro en "${activeTab}"` : `Editar Registro en "${activeTab}"`}
              </h4>
              <button
                onClick={() => {
                  setIsEditModalOpen(false);
            ...
            {modalError && (
              <div className="bg-rose-50 dark:bg-rose-950/40 text-rose-800 dark:text-rose-200 p-3 rounded-xl border border-rose-200 dark:border-rose-800 text-xs flex items-center gap-2 font-medium">
            ...
            <div className="space-y-2">
              <label className="block text-xs font-semibold text-gray-700 dark:text-zinc-300">
                Objeto JSON del registro (Edite los campos según sea necesario):
              </label>
              <textarea
                value={editJsonText}
                onChange={(e) => setEditJsonText(e.target.value)}
                rows={12}
                className="w-full p-3 font-mono text-xs bg-gray-950 text-emerald-400 rounded-xl border border-gray-800 focus:outline-none focus:ring-2 focus:ring-indigo-500 leading-relaxed"
              />
            </div>
            ...
            <div className="flex justify-end gap-2 pt-2 border-t border-gray-100 dark:border-zinc-800">
              <button
                ...
                className="px-4 py-2 bg-gray-100 hover:bg-gray-200 dark:bg-zinc-800 dark:hover:bg-zinc-700 text-gray-700 dark:text-zinc-300 font-semibold rounded-xl text-xs transition cursor-pointer"
              >
                Cancelar
              </button>
              <button
                ...
                disabled={savingRow}
                className="px-5 py-2 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-xl text-xs transition cursor-pointer shadow-md flex items-center gap-1.5"
              >
                <Save className="w-4 h-4" />
                {savingRow ? 'Guardando en Supabase...' : (isInsertModalOpen ? 'Insertar en Supabase' : 'Guardar Cambios')}
              </button>
            </div>
          </div>
        </div>
